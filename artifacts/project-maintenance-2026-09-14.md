...
| GPS 照片查看器（公开夹具） | 508.6 KiB | 424.5 KiB | 630 / 520 KiB |

上游升级使 MapLibre 合计 gzip 比旧 360 KiB 门槛高约 19 KiB，因此该门槛调为 390 KiB。补测现有 389 张照片时，完整 manifest 为 104.4 KiB gzip，GPS 路由合计为 612.4 KiB，超过原 600 KiB 门槛；GPS 聚合预算同步增加相同的 30 KiB 至 630 KiB。所有 Brotli 预算、首页预算和地图路由预算保持原值。回归测试保证 Worker 缺失、未计入路由或意外预缓存都会失败。
